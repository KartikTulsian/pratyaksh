...
class _CreateStudentScreenState extends State<CreateStudentScreen> {
  ...
  Future<void> _createStudent() async {
    ...
    try {
      ...
      final String dept = deptController.text.trim();
      final int? year = int.tryParse(yearController.text.trim());
      final int? semester = int.tryParse(semesterController.text.trim());
      final String password = passwordController.text.trim();

      if (year == null || semester == null) {
        setState(() => isLoading = false);
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Year and Semester must be valid numbers")),
        );
        return;
      }
          ...
          builder: (_) => AlertDialog(
            ...
            title: const Text(
              ...
              content: Column(
                ...
                children: [
                  const Text("Student profile created successfully."),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Add Student",  style: TextStyle(fontWeight: FontWeight.w600)),
